...
namespace Invector.CharacterController
{
    public class vThirdPersonInput : MonoBehaviour
    {
        ...
        {
            float Hro = Vector3.Magnitude(new Vector2(Input.GetAxis("Vertical"), Input.GetAxis("Horizontal")));
            if (Input.GetKey(sprintInput) && Mathf.Abs(Hro) > 0.2f )
                cc.Sprint(true);
            else if (Input.GetKeyUp(sprintInput))
                cc.Sprint(false);
            else if(Mathf.Abs(Hro)<0.2f)
                cc.Sprint(false);
